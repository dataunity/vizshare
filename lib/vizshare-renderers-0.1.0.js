...
(function ($, vegaHelper) {
    var renderFuncBar = function (selector, dataHelper, vizSettings) {
            ...
            "data": [
                {
                    "name": dataSetRef,
                    "url": dataset.url,
                    "format": {"type": "csv", "parse": {"__fieldYAxis__": "number"}}
                }
            ],
            "scales": [
                {
        ...
        // Do fieldYAxis substitution: when specifying which fields in the data should
        // be parsed as a number, the field name can't be set as the key in javascript.
        // use substitution here to replace key placeholder with real key.
        var dataset = null;
        var keysToDelete = [];
        for (var i=0; i < vegaSpec.data.length; i++) {
            dataset = vegaSpec.data[i];
            keysToDelete = [];
            for (var key in dataset.format.parse) {
                if (key === "__fieldYAxis__") {
                    dataset.format.parse[fieldYAxis] = dataset.format.parse["__fieldYAxis__"];
                    keysToDelete.push(key);
                }
            }
            for (var j = 0; j < keysToDelete.length; j++) {
                delete dataset.format.parse[keysToDelete[j]];
            }
        }
...
(function ($, vegaHelper) {
    var renderFuncPie = function (selector, dataHelper, vizSettings) {
            ...
            "data": [
                {
                    "name": dataSetRef,
                    "url": dataset.url,
                    "format": {"type": "csv", "parse": {"__valueField__": "number"}},
                    "transform": [
        ...
        // Do valueField substitution: when specifying which fields in the data should
        // be parsed as a number, the field name can't be set as the key in javascript.
        // use substitution here to replace key placeholder with real key.
        var dataset = null;
        var keysToDelete = [];
        for (var i=0; i < vegaSpec.data.length; i++) {
            ...
            keysToDelete = [];
            for (var key in dataset.format.parse) {
                if (key === "__valueField__") {
                    dataset.format.parse[valueField] = dataset.format.parse["__valueField__"];
                    keysToDelete.push(key);
                }
            }
            for (var j = 0; j < keysToDelete.length; j++) {
